...
if [ "$DEPLOY" == "true" ]; then
    ...
    docker run -u `id -u $USER` --name doe -d -p 8989:8989 -e LD_LIBRARY_PATH='/opt/conda/bin/../lib' -v $CWD:/appoptdes doe -e VIRTUAL_HOST=doe.synbiochem.co.uk 
else
    # Run container with user's uid to avoid permission issues when updating the repository
    docker run -u `id -u $USER` --name doe -d -p 8989:8989 -e LD_LIBRARY_PATH='/opt/conda/bin/../lib' -v $CWD:/appoptdes doe 
fi
